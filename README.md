...
* [`src/cljs/hs_test_front/`](src/cljs/hs_test_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/hs_test_front/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `hs_test_front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/hs_test_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns hs-test-front.example
  (:require [hs-test-front.config :as config])
